...
import (
	...
	mock_credentials "github.com/level11consulting/ocelot/common/credentials"
...
func TestGuideOcelotServer_BuildRepoAndHash(t *testing.T) {
	...
	t.Run("no hash build test", func(t *testing.T) {
		...
		mockz.handler.EXPECT().GetBranchLastCommitData("level11consulting/ocelot", "banana").Return(&pb.BranchHistory{Hash: "123", Branch: "banana"}, nil)
		mockz.handler.EXPECT().GetFile("ocelot.yml", "level11consulting/ocelot", "123").Return(ocelot, nil)
		...
		request := &pb.BuildReq{AcctRepo: "level11consulting/ocelot", Branch: "banana"}
		streamer := &buildserv{}
		gos.BuildRepoAndHash(request, streamer)
		expectedLines := []string{
			"Searching for VCS creds belonging to level11consulting/ocelot...",
			"Successfully found VCS credentials belonging to level11consulting/ocelot ✓",
			...
			"Retrieving ocelot.yml for level11consulting/ocelot...",
			"Successfully retrieved ocelot.yml for level11consulting/ocelot ✓",
			"Validating and queuing build data for level11consulting/ocelot...",
			"Build started for 123 belonging to level11consulting/ocelot ✓",
		}
	...
	t.Run("test building with a branch and a hash", func(t *testing.T) {
		request := &pb.BuildReq{AcctRepo: "level11consulting/ocelot", Branch: "banana", Hash: "123"}
		mockz.rc.EXPECT().GetConsul().Return(consl).Times(1)
		mockz.rc.EXPECT().GetVault().Return(vlt).Times(1)
		mockz.rc.EXPECT().GetCred(mockz.store, pb.SubCredType_BITBUCKET, "BITBUCKET_shankj3", "shankj3", false).Return(cred, nil).Times(1)
	...
	t.Run("invalid ocelot yml", func(t *testing.T) {
		mockz.rc.EXPECT().GetConsul().Return(consl).Times(1)
		consl.EXPECT().GetKeyValue("ci/werker_build_map/123").Return(nil, nil)
		request := &pb.BuildReq{AcctRepo: "level11consulting/ocelot", Branch: "master", Hash: "123"}
		mockz.store.EXPECT().RetrieveLatestSum("123").Return(&pb.BuildSummary{}, storage.BuildSumNotFound("123"))
		mockz.rc.EXPECT().GetCred(mockz.store, pb.SubCredType_BITBUCKET, "BITBUCKET_shankj3", "shankj3", false).Return(cred, nil).Times(1)
...
func TestGuideOcelotServer_BuildRepoAndHash_previouslybuilt(t *testing.T) {
	...
	request3 := &pb.BuildReq{AcctRepo: "level11consulting/ocelot", Hash: "ks72bas"}
	...
	mockz.handler.EXPECT().GetFile("ocelot.yml", "level11consulting/ocelot", "ks72basasdfasdf").Return(ocelot, nil)
	mockz.handler.EXPECT().GetChangedFiles("level11consulting/ocelot", gomock.Any(), gomock.Any()).Return([]string{"ocelot.yml"}, nil)
	mockz.handler.EXPECT().GetCommit("level11consulting/ocelot", "ks72basasdfasdf").Return(&pb.Commit{Message:"hi", Hash: "ks72basasdfasdf" +
		""}, nil).Times(1)
	...
	expected := []string{
		...
		"Successfully retrieved ocelot.yml for level11consulting/ocelot ✓",
		"Validating and queuing build data for level11consulting/ocelot...",
		"Build started for ks72basasdfasdf belonging to level11consulting/ocelot ✓",
	}
	...
	request2 := &pb.BuildReq{AcctRepo: "level11consulting/ocelot", Hash: "ks72bas"}
	streamer = &buildserv{}
...
func TestGuideOcelotServer_BuildRepoAndHash_branchNotfound(t *testing.T) {
	gos, ctl, mockz := setupMockedGuideOcelot(t)
	defer ctl.Finish()
	req := &pb.BuildReq{AcctRepo: "level11consulting/ocelot", Branch: "schmorgazbord"}
	cred := &pb.VCSCreds{ClientSecret: "1", ClientId: "2", Identifier: "BITBUCKET_shankj3", AcctName: "shankj3", SubType: pb.SubCredType_BITBUCKET, TokenURL: "http://token"}
	mockz.rc.EXPECT().GetCred(mockz.store, pb.SubCredType_BITBUCKET, "BITBUCKET_shankj3", "shankj3", false).Return(cred, nil).Times(1)
	mockz.handler.EXPECT().GetBranchLastCommitData("level11consulting/ocelot", "schmorgazbord").Return(nil, models.Branch404("schmorgazbord", "level11consulting/ocelot")).Times(1)
	streamer := &buildserv{}
	errmsg := "Branch schmorgazbord was not found for repository level11consulting/ocelot"
