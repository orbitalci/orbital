...
func Register(consulete *consul.Consulet, ip string, grpcPort string, wsPort string) (werkerId uuid.UUID, err error) {
	...
	if err = consulete.AddKeyValue(build.MakeWerkerIpPath(strId), []byte(ip)); err != nil {
		return
	}
	if err = consulete.AddKeyValue(build.MakeWerkerGrpcPath(strId), []byte(grpcPort)); err != nil {
		return
	}
	if err = consulete.AddKeyValue(build.MakeWerkerWsPath(strId), []byte(wsPort)); err != nil {
		return
	}
	return
}
...
func UnRegister(consulete *consul.Consulet, werkerId string) error {
	err := consulete.RemoveValues(build.MakeWerkerLocPath(werkerId))
	return err
}


func RegisterStartedBuild(consulete *consul.Consulet, werkerId string, gitHash string) error {
	if err := consulete.AddKeyValue(build.MakeBuildMapPath(gitHash), []byte(werkerId)); err != nil {
		return err
	}
	return nil
}
...
func RegisterBuild(consulete *consul.Consulet, werkerId string, gitHash string, dockerUuid string) error {
	...
	err := consulete.AddKeyValue(build.MakeDockerUuidPath(werkerId, gitHash), []byte(dockerUuid))
	return err
}
...
func RegisterBuildSummaryId(consulete *consul.Consulet, werkerId string, gitHash string, buildId int64) error {
	...
	err := consulete.AddKeyValue(build.MakeBuildSummaryIdPath(werkerId, gitHash), []byte(str))
	return err
}
...
func RegisterBuildStage(consulete *consul.Consulet, werkerId string, gitHash string, buildStage string) error {
	...
	err := consulete.AddKeyValue(build.MakeBuildStagePath(werkerId, gitHash), []byte(buildStage))
	return err
}
...
func RegisterStageStartTime(consulete *consul.Consulet, werkerId string, gitHash string, start time.Time) error {
	...
	err := consulete.AddKeyValue(build.MakeBuildStartpath(werkerId, gitHash), []byte(str))
	return err
}
