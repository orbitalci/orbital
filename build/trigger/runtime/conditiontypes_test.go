...
func TestBranchCondition(t *testing.T) {
	...
	triggerData := &pb.ChangesetData{Branch: "borg"}
...
func TestTextCondition(t *testing.T) {
	...
	td := &pb.ChangesetData{CommitTexts: []string{"text1 goes here and that's real nice", "text2345 is also here!!!", "but we don't have the last nonsensical boi"}}
	...
	if !text.PassesMuster(&pb.ChangesetData{CommitTexts: []string{"text1 geez text2345 t82nsla7812l"}}) {
...
func TestFilepathCondition(t *testing.T) {
	...
	if !fp.PassesMuster(&pb.ChangesetData{FilesChanged: changelist}) {
	...
	if !fp.PassesMuster(&pb.ChangesetData{FilesChanged: robustList}) {
