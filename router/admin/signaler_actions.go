...
func (g *guideOcelotServer) BuildRepoAndHash(buildReq *pb.BuildReq, stream pb.GuideOcelot_BuildRepoAndHashServer) error {
	...
	//	}
	//}
	checkData := build.NewViable(branch, buildConf.Branches, nil, buildReq.Force)
	if err = g.getSignaler().CheckViableThenQueueAndStore(buildReq.Hash, token,  branch, buildReq.AcctRepo, buildConf, checkData); err != nil {
